...
- [ ] Populate only source- and license-approved Plant ID, IPM/diagnosis, and commerce-pack records, run their contract tests, then seed a networked test/staging environment. The governed Facility pack is already seeded and verified above.
...
103. Facility product, product-batch, order, and inventory source links now preserve ids in `/home/facility/inventory/[id]`, so stock alerts, notifications, tasks, and operational references can open the exact inventory item instead of the facility inventory root.
...
- Completed and live-verified: the upgrade screen now shows comparison copy for who-it-is-for, what-it-unlocks, and what Stripe does next for Pro, Commercial, and Facility. Unit coverage passes; the live retest passed on 2026-07-29 at `/home/personal/upgrade`.
...
- Payment-safety correction deployed on 2026-07-31: the recipient fields were frontend-only and the authoritative backend ignored them, so an ordinary Checkout would have granted the plan to the payer without fulfilling the gift. Backend PR `#96` / main `adb08e0` now rejects gift mode before Stripe and reports `giftCheckoutConfigured: false`; frontend `24831d73` disables gift checkout with truthful unavailable copy while preserving buy-for-me checkout. Recipient claim, delivery email, activation, privacy, and recipient-side acceptance remain open; gift subscriptions are not complete. Frontend `53c4495a` also removed the raw `upgrade/index` heading from the live Personal upgrade page. See `docs/qa/GIFT_SAFETY_UPGRADE_AND_NOTIFICATION_THEME_PRODUCTION_EVIDENCE_2026-07-31.md`.
...
- Completed and live-verified: Discover points live browsing to `/lives`, the public Lives screen renders campaign-linked opportunities, upcoming sessions, live-now, and replay sections instead of a plain join list, and Forum/Q&A exposes a dedicated Lives card that routes to the same browsing surface. Live browser verification on 2026-07-29 confirmed the campaign-framed browsing layout and empty-state counts.
...
- Released through frontend merge `02edbdab697c5f578837eca92561bfe34b49ccd3`: the Commercial command center shows a launch-readiness checklist for public slug, product count, and storefront status, plus clearer draft-shell guidance when slug or products are missing. Focused coverage and both main workflows passed. Follow-up merge `a6530eec1a20d6ee42f3f21980b29e3b63a8b14f` restored Commercial eligibility for the connected Facility-plan owner without granting invited Personal Pro Facility members Commercial access. The 2026-08-03 exact-SHA production desktop pass entered Commercial and verified Dashboard, Storefront, Feed, Forum, More, and Profile with their expected headings, six labeled tabs, zero horizontal overflow, and zero fresh console errors. Commercial record-mutating, payment, public-handoff, mobile/device, and full role acceptance remain open.
...
- Personal, Commercial, and Facility tab layouts hide non-tab routes with Expo Router `href: null` instead of invisible tab buttons. Frontend merge `02edbdab697c5f578837eca92561bfe34b49ccd3` shipped the implementation, and follow-up merge `a6530eec1a20d6ee42f3f21980b29e3b63a8b14f` completed current Commercial role-loaded desktop acceptance. Source-contract and rendered 375/390-pixel tests verify six visible labels per workspace; those are local narrow-viewport tests, not production-mobile or physical-device evidence. Exact-SHA production desktop checks confirmed six labeled tabs without blank slots on Personal, Commercial, and Facility. Narrow-width production Browser and physical-device acceptance remain open.
...
- Released through frontend merge `02edbdab697c5f578837eca92561bfe34b49ccd3`: the shared night palette uses the lighter blue-gray surface/text/link system, and the common back button renders on a visible surface-muted fill instead of disappearing into the page background.
  - The same release moved shared grow widgets and fallback shells to the shared palette, including the grow selector, stage slider, plant save modal, plant card, skeleton loaders, both error-boundary surfaces, and the shared AppShell / CommercialBanner / ContextBar chrome. Workspace tab layouts no longer mix `href` with hidden `tabBarButton` routes on web. Personal Home/Grows/AI/Courses/Diagnose/Profile/Schedule and Facility AI/Profile exact-SHA checks loaded without browser errors, but the all-route, all-role responsive/accessibility acceptance remains open.

692. The consolidated navigation/visual/feed/AI ownership release is live on frontend merge `02edbdab697c5f578837eca92561bfe34b49ccd3` (Render `dep-d9nr95c9v7es73ckembg`). Both main workflows passed. The first exact-SHA Personal Forum retest found a real web-only `CSSStyleDeclaration` crash from raw style arrays on two linked quick actions; hotfix PR `#286` merged as `1e595a5dfce64cad5ab325445bdb70238b08c46b`, both main workflows passed, and Render `dep-d9nt07oae00c73fiefcg` became Live. The final Personal Forum retest loaded the discussion content, both quick workflows, and six tabs with zero browser errors. Exact-SHA Personal Home, Grows, AI, Courses, Diagnose, Profile, and Schedule plus Facility AI/Profile checks also passed without browser errors; one real Personal Pro Ask AI request deducted and persisted exactly one credit (`100 / 100 -> 99 / 100`, one billed request, zero refunds). Current Commercial role-loaded acceptance, the remaining role matrix, responsive/accessibility coverage, final screenshots/video, and independent outside-user validation remain open.
...
- 2026-08-03 follow-up: workspace-access source `5d65bcca54fd755dfefc43c30c58e817c0a0edde` merged through PR `#287` as `db816c6fdf22c9e1f87388ef8f81ea307979b67a`, then Commercial Forum heading hotfix `ed8a66e3b3aba1951483e9fca7f8feb1991a8054` merged through PR `#288` as final main `a6530eec1a20d6ee42f3f21980b29e3b63a8b14f`. Both main workflows passed for both merge SHAs. The connected Facility-plan owner account exposed all three eligible workspaces; exact-SHA production desktop checks passed six Personal routes, six Commercial routes, Facility Dashboard and six operational nested routes, shared Courses/Forum, and Personal Field Study recovery with expected headings, zero horizontal overflow, and zero fresh console errors. Local regression passed all 89 frontend batches, 21 backend suites / 193 tests, and desktop plus narrow/mobile-emulation Playwright; local emulation does not close production-mobile, physical-device, keyboard/focus, screen-reader, screenshot/video, role-matrix, payment/source-data, or outside-user acceptance. See `docs/qa/WORKSPACE_ROUTE_RECOVERY_PRODUCTION_EVIDENCE_2026-08-03.md`.
