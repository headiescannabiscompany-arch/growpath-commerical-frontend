...
- [ ] Replace the ordinary Forum-card `Open discussion` page jump with an accessible expand/collapse arrow that loads comments and reply controls directly below the post. Keep the dedicated detail URL only for direct links and advanced context, and reuse one shared inline-discussion pattern anywhere Forum posts are previewed.
