...
1. Retrieve the selected grow/plant/workspace, relevant records, media and conversation turns.
   ...
   After a successful review, retain a bounded manifest for every diagnostic crop actually sent to the provider: source evidence ID and image index, crop strategy and source bounds when available, output dimensions, media type, and a cryptographic digest of the exact derived bytes. Authorized users may view, save, or export those exact source-bound views by regenerating them from the retained original and verifying the digest. Never present a regenerated mismatch, an uninspected client crop, or a count-only response as inspected visual evidence. Exports must repeat that crops are supplemental views of original pixels rather than additional photos, sites, samples, or independent observations.
