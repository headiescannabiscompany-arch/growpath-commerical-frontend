...
Paid-plan offers must use the authenticated account's actual trial eligibility rather than generic promotional language. If a one-time trial is still available, state its duration, payment-method requirement, and when paid billing begins. If the account has already used the trial or trials are disabled, state that checkout will bill the displayed price when completed and require a second explicit, price-labeled confirmation before opening Stripe. A successful return must refresh the account session and visibly confirm completion; a canceled return must visibly confirm that the checkout submitted no new payment.
