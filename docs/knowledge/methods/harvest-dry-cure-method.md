...
The minimum four-photo Harvest set must receive enlarged center and attention-selected inspection views for every original photo, including the wider or brightest final image. Larger sets may bound derived views for provider reliability, but every untouched original remains present at high detail. Never silently omit diagnostic enlargement from one member of the minimum set and then characterize that photo as fully zoom-inspected.

Every AI image workflow must preserve and inspect the untouched original-resolution image and may add server-created center and attention-selected diagnostic crops from the original pixels. These enlarged views supplement the original instead of replacing or downscaling it. They remain bound to their source image and never count as an additional photo, sample site, observation, role, or independent evidence. Crop creation failure must fall back to the untouched original rather than rejecting the upload.
...
When qualified trichome evidence is missing, Harvest Readiness returns `insufficient_evidence` and no dated estimate. It must not translate missing evidence into a generic three-to-seven-day recommendation, suggested check date, harvest-decision task, or dry/cure preparation deadline. It may create evidence-capture and maturity-observation tasks only.
