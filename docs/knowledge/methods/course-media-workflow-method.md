...
An authenticated course owner uses explicit Publish Course and Unpublish Course actions. Publishing applies the media and plan checks above. Unpublishing makes the course a private draft without deleting its lessons or payment history. Do not expose submit, approve, or reject actions unless a real moderated-review backend workflow and role policy are implemented end to end.
