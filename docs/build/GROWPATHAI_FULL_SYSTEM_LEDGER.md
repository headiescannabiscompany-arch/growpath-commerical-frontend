...
- Hardened the V1 feature/backend matrix quality contract: all 226 rows now carry explicit `rowStatus` labels (`canonical`, `compat_alias`, `supporting_operation`, `deprecated`, `planned`), and validation rejects duplicate feature IDs, malformed modes, unexplained duplicate UI routes, user-visible `unknown` mode rows, and supporting operations without canonical owners. The 65 formerly planned internal routes are inventoried backend operations mapped to existing product rows; zero planned rows remain.
...
- Replaced the Run-To-Run Comparison demo-row/arbitrary-winner implementation with an owned saved-history service and shared Personal Tools/grow workspace. The workflow compares two to five grows against an explicit reference and equivalent scope, counts only scoped evidence, excludes synthetic telemetry, preserves missing values, limits any objective leader to the selected comparable metric, and saves ToolRun/log/reviewed-task actions. Verification passed for the focused frontend flow, 32 nested tool-router tests, the database-backed ownership/persistence route test, frontend lint, the complete guard chain, 295 frontend suites / 1,121 tests, and production web export/build. The backend repository's unrelated whole-repo formatting debt and legacy Mongo setup timeouts remain outside this slice.
