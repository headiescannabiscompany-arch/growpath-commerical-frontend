...
- [~] Verify subscription checkout end to end: Commercial live Checkout, success return, webhook-confirmed entitlement, refund, cancellation scheduling, and no-future-invoice state are proven. Frontend PR `#240`, merge `809a820625f7edcb9351d93f255a6f98cfc287f7`, now uses the account's actual trial eligibility, requires a second price-labeled confirmation before immediate billing, refreshes the account on success, and shows clear success/cancellation feedback; production deployment `dep-d9j203cvikkc73d680o0` passed live Browser verification without creating another Checkout. Decide whether the one-time trial remains account-wide or becomes plan-specific, then verify the terminal August 26 expiry/downgrade. Pro and Facility settlement remain separate acceptance cases.
  - 2026-07-26 policy decision and local implementation: each account receives at most one separate 30-day trial for Pro, Commercial, and Facility; use of one does not consume the others, and legacy Boolean-only trial history maps to Pro. Production deployment and live settlement/repeat-protection evidence remain open.
