...
- [x] Production Personal Pro saved-photo diagnosis transport and billing passed: frontend merge `e67e5fb4091350a3953f4bbbab97256f022af990` exposed and previewed private grow photos, backend merge `a78d219589a8c434e2e9ce8e58c663af52708570` enforced ownership and supplied the image-capable diagnosis request, and one live request persisted the exact three-credit deduction (`93 -> 90`) with zero refunds.
- [ ] Production diagnosis acceptance still requires fresh file upload, failed-request refund behavior, plant/crop context, useful discriminating follow-up and outcome capture, log/task writeback, saved-result reopening, and independent accuracy review. The saved-photo retest had no plant selector and returned crop identity `unspecified`, so transport/provider evidence is not final diagnostic-quality acceptance.
...
- [~] Run the two-track user-type verification in `docs/qa/USER_TYPE_LOOP_CLOSURE_MATRIX_2026-07-16.md`: close every persisted/downstream/permission/delivery loop while separately reviewing product polish, Free-plan invitation, and outside-user validation.
  ...
  - 2026-07-20 Personal Pro production evidence: one provider-backed Ask AI request deducted and persisted exactly one credit (`94 -> 93`) with zero refunds. The invalid current-plan Pro upgrade was removed in frontend PR `#64`; merge `cc822f8dbc242c08279aeb9089628b85010c3c0a` was live-retested at 8:33 PM ET. Frontend PRs `#76`/`#77` and backend PR `#34` later enabled ownership-scoped reuse of real grow photos; one live OpenAI-backed diagnosis deducted and persisted exactly three credits (`93 -> 90`) with zero refunds. Saved-photo transport/provider/ledger passed, while fresh upload, failure/refund, plant/crop context, follow-up/writeback/reopening, lifecycle, billing, accessibility, and exported-recording checks stay open; see `docs/qa/PERSONAL_PRO_AI_CREDIT_EVIDENCE_2026-07-20.md`.
