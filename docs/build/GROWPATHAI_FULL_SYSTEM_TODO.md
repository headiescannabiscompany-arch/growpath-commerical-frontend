...
- [deferred-owner-input] Real Facility Viewer invitation acceptance, the automatic Facility handoff/read-only-role production retest, and owner-approved temporary Staff/Viewer alias cleanup are parked with the final owner-input pass. The session repair is already live on frontend `ac58eb4c` and backend `41d50b69`; do not interrupt other actionable verification for this item unless the owner explicitly reopens it.
