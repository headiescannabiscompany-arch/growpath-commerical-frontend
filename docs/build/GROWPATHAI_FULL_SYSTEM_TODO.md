...
- [x] Production Personal Pro saved-photo diagnosis transport, visual context, and billing passed: frontend merges `e67e5fb4091350a3953f4bbbab97256f022af990` and `6ed88c43dcda7fd0cc215fef5cf80ace973baff9` exposed the private grow photo and its visual evidence, backend merges `a78d219589a8c434e2e9ce8e58c663af52708570` and `29db80b439f7f6e2d52e227515f245c5a85b144a` enforced ownership and returned structured visual context, and two live requests each persisted the exact three-credit deduction (`93 -> 90 -> 87`) with zero refunds. The second request identified `Cannabis` / `Cannabis sativa` at high confidence, did not infer a strain, displayed evidence and photo-improvement guidance, and asked a useful follow-up.
- [ ] Production diagnosis acceptance still requires fresh file upload, failed-request refund behavior, follow-up answer/refinement and outcome capture, log/task writeback, saved-result reopening, and independent accuracy review. The grow's live Plants page showed `No plants yet`, so absence of a plant selector was expected rather than a hidden-record defect.
...
- [~] Run the two-track user-type verification in `docs/qa/USER_TYPE_LOOP_CLOSURE_MATRIX_2026-07-16.md`: close every persisted/downstream/permission/delivery loop while separately reviewing product polish, Free-plan invitation, and outside-user validation.
  ...
  - 2026-07-20/21 Personal Pro production evidence: one provider-backed Ask AI request deducted and persisted exactly one credit (`94 -> 93`) with zero refunds. The invalid current-plan Pro upgrade was removed in frontend PR `#64`; merge `cc822f8dbc242c08279aeb9089628b85010c3c0a` was live-retested at 8:33 PM ET. Frontend PRs `#76`/`#77` and backend PR `#34` later enabled ownership-scoped reuse of real grow photos; the first live OpenAI-backed diagnosis deducted and persisted exactly three credits (`93 -> 90`) with zero refunds. Backend PR `#35` and frontend PR `#79` then returned and displayed draft visual identity and photo-quality context. After both deployments were live at 12:25 AM ET, the same saved bud photo was identified as `Cannabis` / `Cannabis sativa` at high confidence without strain inference, received exact replacement-photo guidance and a discriminating follow-up, and deducted exactly three credits again (`90 -> 87`; 8 -> 9 billed requests; zero refunds). Fresh upload, failure/refund, follow-up/writeback/reopening, lifecycle, billing, accessibility, and exported-recording checks stay open; see `docs/qa/PERSONAL_PRO_AI_CREDIT_EVIDENCE_2026-07-20.md`.
