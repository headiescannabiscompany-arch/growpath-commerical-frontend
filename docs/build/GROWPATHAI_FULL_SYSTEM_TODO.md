...
- [x] Facility SOP Library implementation added: eight setpoint-free standard starters, owner/manager-only supporting-document uploads, executable checklist enforcement, Facility-owned attachment validation, preserved revisions, non-destructive retirement, and read-only member access have focused frontend and backend database coverage. The full frontend regression passed 312 suites / 1,231 tests, all repository guards passed, the production export succeeded, the backend core/system regression passed 25 suites / 172 tests, and the final SOP/AI/compatibility database pass succeeded 3 suites / 25 tests. Frontend merge `abc60a123a34765014aa502dca26c82f8e99738a` and backend merge `bea6633831f4c96594096832053839d2d5c47cce` passed post-merge workflows and were live when a signed-in production Owner completed the non-destructive library/readiness pass. Source version 2 is live on frontend `aaf68b2fe3380935f1faac9fa7ef6b17759934af` and backend `ff32ec14b27fc4c3fb75643e3e2446929dbec5a1` with a guard against Facility-only wording in shared starters. Viewer and deliberate upload/create/revise/retire production evidence remain open; see `docs/qa/SOP_LIBRARY_AI_RECOMMENDATIONS_PRODUCTION_EVIDENCE_2026-07-25.md`.
