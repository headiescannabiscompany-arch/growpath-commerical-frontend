...
- [~] Verify Storefront, Feed/Campaigns, Forum/Q&A, Lives, Orders, Analytics, Product Lines, Batches/Lots, Trials, and Inventory navigation names and hierarchy are clear and not duplicative.
  ...
  - 2026-07-24 Commercial production navigation and record-linking polish is live through frontend commit `4778c51b911cf6fbfb0704ffdb9e61f8071c75f7`: compact navigation retains every omitted workspace under More; wide bottom-tab labels are centered and fully visible without empty icon rows or fallback symbols; Product Lines, Product Batches, Product Trials, Inventory Support, and Product Trial Evidence Runs expose one page heading plus structured workflow sections; Inventory Support creation is a named action; Product Trial, Product Batch, and Inventory Support creation prefer readable owner-scoped record choices; and compact campaign rails no longer overlap or intercept form controls. The signed-in production pass created no records. Render's obsolete duplicate Node frontend service was confirmed as the source of repeated false failed deploys and had automatic deploys turned off; the production Static Site remained live and unchanged. Intentional create/edit/reload coverage, real public-user handoffs, remaining responsive visual QA, screenshot/video capture, and independent acceptance remain open; see `docs/qa/COMMERCIAL_PRODUCTION_WORKFLOW_NAVIGATION_EVIDENCE_2026-07-24.md`.
