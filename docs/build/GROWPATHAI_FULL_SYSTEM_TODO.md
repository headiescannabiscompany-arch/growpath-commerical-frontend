...
- [~] Live Stripe configuration and checkout-session creation are verified. On 2026-07-26 the production account showed one real Pro trial for `headiescannabiscompany@gmail.com` at $10/month through August 15, and GrowPath created and safely canceled live Commercial ($50/month) and Facility ($100/month) Checkout sessions. Two replacement webhook destinations now listen to `Your account` rather than the incorrect `Connected accounts` scope: `/api/payments/webhook` receives the six subscription/payment events and `/api/facility-billing/webhook` receives the three Facility lifecycle events; their signing secrets were deployed through Render deployment `dep-d9j0u6btqb8s739j566g`. An intentional live payment is still required for new webhook-delivery, entitlement/enrollment, refund, and dispute evidence. See `docs/qa/PRODUCTION_EMAIL_STRIPE_EVIDENCE_2026-07-26.md`.
