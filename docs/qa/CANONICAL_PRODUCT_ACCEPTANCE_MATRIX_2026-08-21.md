...
| P-08 | Harvest Readiness works from ordinary phone media with sample ranges and reasons to harvest/wait                     | partial: max-80 extraction/restore, 512 MiB guard, standalone context, exact 80-image/7-batch/7-credit quote and one durable pre-dispatch operation are live; the canonical ownership/lifecycle backend is focused-accepted and the matching frontend passes static reconciliation; exact-lock CI/export, backend-first deployment, same-operation recovery, paid result, full-resolution role sufficiency, private Feed review/export and live deletion/privacy/cleanup acceptance remain open |
