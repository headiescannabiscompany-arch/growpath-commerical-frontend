...
receipts, private selected-frame/result sharing and guarded deletion. P-08 now has one frozen
ownership/state-transition packet: Personal and Commercial records are account-owned; Facility
records are Facility-owned with the initiating actor retained only as audit provenance; private
Feed drafts never enter public projections; legal holds, exact replay, saved-result deletion,
account deletion and Facility-role changes all use the same transaction fences. The exact entity
table, transition table, focused acceptance cells and remaining production gates live in
`P08_HARVEST_READINESS_CANONICAL_EVIDENCE_2026-08-24.md`; route work cannot invent a competing
ownership or cleanup rule.
The original receipt configuration, backend-first max-80 deployment, readiness proof and real
FFmpeg extraction are retained evidence and are not repeated. The current completion path is the
corrected backend, its matching frontend, then one recovery of the existing accepted operation as
specified by the P-08 record. Backend-focused lifecycle acceptance is now closed locally; the
matching frontend is statically reconciled, while its added deletion tests, exact-lock export,
deployment and live recovery remain the named CI/live gates rather than reasons to rebuild P-08.
...
| `P-08`        | Harvest Readiness UI/API, server video extraction/preselection, durable operations/receipts, Saved Runs, private Feed review, export/delete/account-erasure/legal-hold services, method and evaluation | Ordinary-phone range; max-80 technical selection; Standard/Deep quote/restore; color vs structural morphology; Personal/Commercial account ownership vs Facility ownership; exact replay; private share/delete; hold/race fences; optional Grow; exact-SHA live evidence |
...
| `R-05`        | GrowPathAI brand assets, BLVNK model/textile/color records, source SVGs, stitch plan, machine-file register, sew-outs and research-trial/storefront presentation                                                        | Owner-resumed gate; sixteen GrowPathAI-only concepts, returned `DST` plus editable digitizer source, exact-blank sample approval, zero stock/not for sale                                                                                                                  |
