type Utf8Options = { replaceUnpairedSurrogates?: boolean };

function rotateRight(value: number, bits: number) {
  return (value >>> bits) | (value << (32 - bits));
}

export function utf8Bytes(value: string, options: Utf8Options = {}) {
  const result: number[] = [];
  for (let index = 0; index < value.length; index += 1) {
    ...
    if (codePoint >= 0xd800 && codePoint <= 0xdbff) {
      ...
      if (low >= 0xdc00 && low <= 0xdfff) {
        ...
        index += 1;
      } else if (options.replaceUnpairedSurrogates) {
        codePoint = 0xfffd;
      }
    } else if (
      options.replaceUnpairedSurrogates &&
      codePoint >= 0xdc00 &&
      codePoint <= 0xdfff
    ) {
      codePoint = 0xfffd;
...
export function sha256Utf8(value: string, options: Utf8Options = {}) {
  const constants = [
  ...
  const words = utf8Bytes(String(value), options);
