...
export default function FacilityTeamScreen() {
  ...
  const locked = !capabilities?.commercialOrders;
  return (
    <ScreenScaffold
      mode="commercial"
      title="Orders"
      subtitle="View and manage your commercial orders and fulfillment."
      status={locked ? "LOCKED" : "STUB"}
      emptyState={
        locked ? (
          <Pill text="Locked: Your plan does not include Orders." tone="locked" />
        ) : (
          <Pill
            text="No orders found. Orders will appear here when available."
            tone="locked"
          />
        )
      }
    >
      <Section title="Planned features">
        <Card title="Roadmap">
          <Pill text="• Orders list" />
          <Pill text="• Order details" />
          <Pill text="• Fulfillment actions" />
        </Card>
      </Section>
    </ScreenScaffold>
  );
