 ...
 * Prevents typos like "harvestt" from silently 404ing at runtime
 */
type AITool = "harvest";
 ...
 */
export default function HarvestWindowScreen({
  facilityId,
  growId
}: {
  facilityId: string;
      ...
      {!!decision && (
        <View style={styles.card}>
          ...
          <Text style={styles.line}>
            Confidence: {(Number(decision.confidence) * 100).toFixed(0)}%
          </Text>
          <Text style={styles.line}>Earliest: {decision.window?.earliest}</Text>
          <Text style={styles.line}>Latest: {decision.window?.latest}</Text>
...
const styles = StyleSheet.create({
  ...
  empty: { fontSize: 13, opacity: 0.5, fontStyle: "italic", marginTop: 8 }
});
