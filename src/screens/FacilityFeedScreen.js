...
export default function FacilityFeedScreen() {
  ...
  const locked = !capabilities?.facilityFeed;
  return (
    <ScreenScaffold
      mode="facility"
      title="Feed"
      subtitle="See updates, alerts, and activity for your facility."
      status={locked ? "LOCKED" : "STUB"}
      emptyState={
        locked ? (
          <Pill text="Locked: Your plan does not include Feed." tone="locked" />
        ) : (
          <Pill
            text="No feed items yet. Updates will appear here when available."
            tone="locked"
          />
        )
      }
    >
      <Section title="Planned features">
        <Card title="Roadmap">
          <Pill text="• Facility activity feed" />
          <Pill text="• Alerts and notifications" />
          <Pill text="• System messages" />
        </Card>
      </Section>
    </ScreenScaffold>
  );
