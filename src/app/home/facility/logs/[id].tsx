...
export default function FacilityLogDetail() {
  ...
  const load = useCallback(
    async (opts?: { refresh?: boolean }) => {
      ...
      try {
        clearError();
        const res = await apiRequest(endpoints.growlog(facilityId, String(id)));
        setItem(res ?? null);
      } catch (e) {
        handleApiError(e);
      } finally {
  ...
  const title = useMemo(() => (item ? pickTitle(item) : "Log Detail"), [item]);
  ...
  return (
    <ScreenBoundary title={title} showBack backFallbackHref="/home/facility/logs">
      ...
      >
        ...
        {loading ? (
          <View
            accessibilityLabel="Loading facility journal entry"
