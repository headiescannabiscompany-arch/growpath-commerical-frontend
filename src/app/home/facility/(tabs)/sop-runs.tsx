...
function pickId(x: AnyRec): string {
  return String(x?.id ?? x?._id ?? x?.runId ?? x?.uuid ?? "");
}

function pickTitle(x: AnyRec): string {
  return String(x?.name ?? x?.title ?? x?.templateName ?? x?.templateId ?? "SOP Run");
}

function pickSubtitle(x: AnyRec): string {
  const status = x?.status ?? x?.state;
  const started = x?.startedAt ?? x?.createdAt;
  const completed = x?.completedAt;
  const parts = [
    status ? `Status: ${String(status)}` : "",
    started ? `Started: ${String(started)}` : "",
...
export default function FacilitySopRunsTab() {
  ...
  const header = useMemo(() => {
    ...
    return n === 1 ? "1 recent run" : `${n} recent runs`;
  }, [items.length]);

  return (
    <ScreenBoundary title="SOP Runs">
      <View style={styles.container}>
        ...
        <View style={styles.actions}>
          ...
          >
            <Text style={styles.actionText}>Open Runs Index ›</Text>
          </Pressable>
          ...
          <Pressable
            ...
            style={({ pressed }) => [styles.actionBtn, pressed && styles.pressed]}
          >
            <Text style={styles.actionText}>Open Presets ›</Text>
          </Pressable>

          <Pressable
            onPress={() => router.push("/home/facility/sop-runs/start")}
            style={({ pressed }) => [styles.actionBtn, pressed && styles.pressed]}
          >
          ...
          <Pressable
            onPress={() => router.push("/home/facility/sop-runs/compare")}
            style={({ pressed }) => [styles.actionBtn, pressed && styles.pressed]}
          >
            <Text style={styles.actionText}>Compare Runs ›</Text>
          </Pressable>
        </View>
        ...
        {loading ? (
          <View style={styles.loading}>
            <ActivityIndicator />
            <Text style={styles.muted}>Loading SOP runs…</Text>
          </View>
        ) : null}
          ...
          ListEmptyComponent={
            !loading ? (
              <View style={styles.empty}>
                <Text style={styles.emptyTitle}>No SOP runs yet</Text>
                <Text style={styles.muted}>Start a run to see it here.</Text>
              </View>
            ) : null
          }
          renderItem={({ item }) => {
...
const styles = StyleSheet.create({
  ...
  actionText: { fontWeight: "900", opacity: 0.8 },
