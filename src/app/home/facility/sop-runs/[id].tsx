...
export default function FacilitySopRunDetailRoute() {
  ...
  const load = useCallback(
    async (opts?: { refresh?: boolean }) => {
      ...
      try {
        const path =
          (endpoints as any)?.sopRun?.(facilityId, id) ??
          `/api/facility/${encodeURIComponent(String(facilityId))}/sop-runs/${encodeURIComponent(id)}`;

        const res = await apiRequest(path, { method: "GET" });
        setItem(res ?? null);
      } catch (e) {
        resolved.handleApiError(e); // Handle API error
      } finally {
        setLoading(false);
        setRefreshing(false);
      }
    ...
    <ScreenBoundary title="SOP Run">
      ...
      >
        ...
        <View style={styles.headerRow}>
          <Text style={styles.h1}>SOP Run</Text>
        ...
        {loading ? (
          <View style={styles.loading}>
            <ActivityIndicator />
            <Text style={styles.muted}>Loading SOP run…</Text>
          </View>
        ) : null}
        ...
        <View style={styles.card}>
          {item ? (
            ...
            <Text style={styles.muted}>
              {id ? "No SOP run returned." : "Missing SOP run id in route params."}
            </Text>
          )}
        </View>
