...
export default function PublicStorefrontCourseRoute() {
  ...
  const reconcileCourseCheckout = useCallback(
    async (shouldContinue?: () => boolean) => {
      ...
      try {
        const result = await pollCourseAccessStatus(id, { shouldContinue });
        ...
        if (result.state === "confirmed") {
          ...
          await clearPendingBuyerCheckout("course", id).catch(() => false);
          setCheckoutPending(false);
          setFeedback("The server reports that this checkout did not create access.");
        } else {
          setFeedback(
            "Enrollment is still awaiting server confirmation. GrowPath did not create another Checkout."
          );
        }
        return result;
      } finally {
        if (!shouldContinue || shouldContinue()) setBusy(false);
      }
    },
    [id]
  );
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (checkoutResult === "canceled") {
        ...
        setFeedback("Checkout was canceled. Course access was not changed.");
        return;
      }
      ...
      if (active) await reconcileCourseCheckout(() => active);
    })();
    return () => {
      active = false;
    };
          ...
          {checkoutResult === "success" ? (
            <AppCard>
              ...
              <Text style={styles.bodyText}>
                {checkoutState === "confirmed"
                  ? "GrowPath confirmed active enrollment. You can open the course."
                  : "The return link does not grant access. GrowPath is verifying payment and enrollment with the server."}
              </Text>
              <Link
                href={
            ...
            <AppCard>
              ...
              <Text style={styles.bodyText}>
                GrowPath recovered this course Checkout and is waiting for
                server-confirmed enrollment before creating another payment.
              </Text>
            </AppCard>
          ) : null}
