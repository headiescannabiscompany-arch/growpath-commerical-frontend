...
export async function getVendorMetrics(vendorId) {
  try {
    const res = await client.get(`/vendors/${vendorId}/metrics`);
...
export async function listVendorSoilMixes(vendorId) {
  try {
    const res = await client.get(`/vendors/${vendorId}/soil-mixes`);
    return { success: true, data: res.data };
  } catch (e) {
    return { success: false, message: e.message };
  }
...
export async function createVendorSoilMix(vendorId, data) {
  try {
    const res = await client.post(`/vendors/${vendorId}/soil-mixes`, data);
    return { success: true, data: res.data };
  } catch (e) {
    return { success: false, message: e.message };
  }
