...
export const methodRegistry: GrowPathMethod[] = [
  ...
  method(
    ...
    [
      ...
      "durable provider operations that freeze one redacted provider input and full selected-source manifest before dispatch, reauthorize the same workspace role before credit claim and provider call, and bind one exact credit journal entry",
      "Business Ask over an explicit subset of all seven B-03 record kinds plus independently selected read-only B-02 inventory, using a clearly labeled UTC last-updated date range",
      "server-attested provider-input, source-manifest, provider-output, and normalized-result digests with all material Business Ask content withheld until the result and citation subset verify",
      "operation-bound citation pages that first show only the exact redacted provider projection, then separately label an authorized exact B-03 revision or same-identity mutable B-02 item/lot comparison whose additional fields were not AI input",
      "paged saved Business Ask draft history with exact provider-operation and citation linkage, explicit reviewed/rejected/archive actions, immutable AI content, version conflicts, and retry recovery",
      "Facility Business Ask drafts shared only inside the selected Facility while owner-only current and projected cash remain excluded from every Facility AI source and answer",
      "account, workspace, and Facility-role scoped opaque retry metadata that never stores the raw question, receipt content, extracted fields, contacts, or financial values locally",
    ...
    [
      ...
      "Never dispatch after a role or authorization-scope change, silently redispatch an ambiguous claimed operation, charge a second credit for an idempotent replay, or retain a failed provider reservation.",
      "Never render a Business Ask answer, calculation, recommendation, or citation as usable until the server attestation matches the exact result digest, full selected-source count, and every cited source's metadata.",
      "Never label a full current or revision record as AI input; first verify and show the operation-bound redacted provider projection, and separately label any authorized comparison and fields outside that projection.",
      "Never substitute a current B-03 record for its cited immutable revision, a different inventory identity for a citation, or a lot without its attested parent item.",
      "Never claim an assistant draft is saved for later review unless older drafts remain reachable; review or rejection changes lifecycle evidence without rewriting AI content or citations.",
      "Never send Facility currentCashMinor or projectedCashMinor through Business Ask, including for a Facility owner; show those fields only in the deterministic authorized Cash-Flow view.",
      "Never store a raw Business Ask question, receipt, extracted field, source record, contact, or financial value in client retry metadata.",
      "Absent a preservation hold, confirmed Commercial account deletion must erase account-owned B-03 content after protected storage release and retain only deidentified minimum tombstones; Facility-owned records remain but every deleted person's actor and reviewer references are deidentified.",
  ...
  method(
    ...
    [
      ...
      "contributor-named reusable public park or trip point that remains active only for the current Plant ID workflow, keeps every photo set as a separate ToolRun-backed observation with its own reviewed description and publish action, and lets the Nature globe cluster the distinct findings",
      ...
      "Facility crop setup with explicit start method, plant count, optional establishment and first-harvest planning anchors, and crop-named AI setup help without invented timing",
      ...
      "known legacy observation coordinates staged through a map or validated latitude/longitude entry and persisted only through a separate explicit private-save action",
    ...
    [
      ...
      "Do not infer a shared trip point from dates, file order, proximity, or another observation; reuse it only after the contributor deliberately names and selects the public park/trip point, and never merge separate plant evidence sets into one observation.",
  ...
  method(
    ...
    [
      ...
      "standalone workspace reload restoration of the newest private Harvest source video with only its exact linked generated-frame set, without requiring a Grow or mixing frames from an older standalone video",
      "maturity",
      "telemetry",
      "sensory",
    ...
    [
      ...
      "review ID, exact ordered evidence IDs, provider/model and image detail, set-level quality checks, per-image findings, visible traits, limitations, image count, and exact quoted/charged/refunded credit status",
      ...
      "explicit authorized permanent-result deletion that removes derived views and unreferenced retained frames, offers a separate private-source-video choice, and respects current references and legal or preservation holds",
      ...
      "saved Deep Review recovery that retains the exact operation ID with signed photoAnalysis, restores the succeeded operation and private-draft address without a new dispatch, serializes Prepare New behind an authoritative no-draft check, applies latest-View-wins cancellation, and clears a displayed draft only for an exact matching DELETE receipt",
      ...
      "account-erasure handling that atomically removes Personal or Commercial Harvest ToolRuns, private drafts, operations, and unreferenced evidence while retaining Facility-owned content and deidentifying the deleted Facility actor",
      "workspace-scoped idempotent deletion receipts that let a current Facility owner or manager resume cleanup without the initiating actor while conflict-checking the immutable source-video keep/delete choice",
      ...
      "sanitized account export that preserves user-facing Harvest observations, limitations, guidance, and safe activity facts while excluding provider, storage, operation, usage, evidence, hash, receipt, and immutable-security internals",
      "bounded exact inspection-view reconstruction with pixel, byte, time, actor/workspace rate, concurrency, and identical in-flight request limits, kept separate from permission for a future provider-AI analysis",
    ...
    [
      ...
      "Preserve an optional source video shorter than 10 minutes as non-AI evidence. Scan no more than 600 temporary low-resolution technical candidates, delete rejected files, and retain only a quality-, diversity-, timeline-, and byte-bounded frame set within the combined 80-image ceiling. Each retained frame must pass the same macro, role, focus, lighting, and glare checks. Keep an adjacent comparison frame with its anchor when useful, but never count the comparison as an independent head sample. When the same head or tightly matching region stays diffusely white across adjacent frames while focus, angle, or highlight position changes, retain lower-confidence likely cloudiness with its matched-frame basis; treat blooming, clipped, glowing, flashing, or moving whiteness as glare-obscured. Never count that bounded likely observation alone as a complete distribution or chemical measurement.",
      ...
      "Allow an authorized Personal, Commercial, or Facility workspace to run Harvest Readiness without creating or attaching a grow after structured cannabis/hemp context or an explicit standalone cannabis/hemp flower confirmation; bind standalone evidence and the receipt to that authenticated workspace, keep canonicalGrowId null, retain Facility-credit ownership, and disable only grow-linked history, task, and batch actions. The confirmation must not create a Grow, change global content visibility, publish evidence, or bypass authorization.",
      ...
      "Never delete a saved Harvest result, derived view, retained frame, or source video without current workspace authority, exact reference checks, and legal/preservation-hold enforcement; fail closed on partial cleanup.",
      ...
      "Never delete Personal or Commercial Harvest operations or evidence while leaving a signed ToolRun packet behind, delete Facility-owned content with the actor's account, export private receipt/provider/security internals, or expose a former Facility actor to Facility-owned result content.",
      ...
      "Never clear a succeeded Deep Review mapping while its private-draft lookup, create, or delete is in flight or while a private draft remains; never let a stale View response replace a newer View or clear a displayed draft from a different DELETE receipt.",
      ...
      "Never race a mutating transaction against a request timeout, return failure while the same write can still commit invisibly, or allow a quote prepared before account deletion or Facility demotion to create a late orphan operation.",
      "Never treat withdrawal of future provider-AI permission as authority to orphan an already signed retained result; authenticated exact local reconstruction remains allowed only while the protected original/result is lawfully retained and digest-matched.",
  ...
  method(
    ...
    [
      ...
      "target-verified stored reports for Forum posts/replies, Feed, storefront, course, video/comments, live/chat, and accounts with evidence snapshots, an administrator inbox, and focused email links",
      "audited leave, hide, restore, soft-remove, reversible cannabis classification, and preservation-first legal escalation",
      ...
      "missing-storefront regulated-commerce guidance that identifies an unmet setup prerequisite, states that no capability decision occurred, and links directly to Storefront setup",
      ...
      "provider-confirmed renewal cancellation with active paid access through the recorded current period end",
    ...
    [
      ...
      "Never replace the ordinary Commercial Grows workspace with Product Trial Evidence Runs or default an unidentified Commercial crop to cannabis.",
      ...
      "Never require a Feed campaign before an eligible published Product or explicitly public approved purchase-intent trial can appear in shared Discover, and never expose draft, hidden, test-only, synthetic, owner-unapproved, or rights-blocked records there.",
      ...
      "Never accept a typed Product lifecycle status, publish an incomplete Product, change incomplete drafts during batch publication, expose draft or hidden Product metadata through a social preview, share a generic site preview when an exact Product preview is available, or describe an external social composer as a completed post.",
      ...
      "Never present a missing Commercial storefront as a failed regulated-commerce review, expose a raw transport code, or show authorization-entry controls before the storefront identity exists.",
      ...
      "Never revoke Personal or Commercial paid access when scheduling renewal cancellation, keep offering repeat cancellation after confirmation, or let a platform Admin target another subscriber.",
  ...
  method(
    ...
    [
      ...
      "Never leave supported Facility AI templates route-only or expose the AI Validation Lab hub entry to managers, staff, or viewers.",
      ...
      "Never revive the legacy standalone EC correction screen as a dosing authority; use the canonical pH/EC interpretation workflow and preserve measured context, uncertainty, and recheck tasks.",
