...
describe("PpfdToolScreen", () => {
  ...
  it("creates light check tasks with shared Schedule metadata", async () => {
    const screen = render(<PpfdToolScreen />);

    fireEvent.press(screen.getByText("Create Light Check Task"));
    ...
    await waitFor(() =>
      expect(mockSaveToolRunAndCreateTask).toHaveBeenCalledWith(
        expect.objectContaining({
          ...
          sourceStage: "ppfd_dli_measurement",
          reminderPlan: expect.objectContaining({
            channels: ["in_app"],
            reminders: [expect.objectContaining({ offsetMinutes: -720 })]
          }),
          description: expect.stringContaining("Target about")
        })
      )
    );
  });
